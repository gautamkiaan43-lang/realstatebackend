...
function extractAllImages(html) {
    ...
    while ((m = regex.exec(html)) !== null) {
        const src = m[1];
        // Only grab property images (not logos/icons — filter by path patterns)
...
async function scrapePropertyDetail(url, index) {
    ...
    if (!title) title = `Immobile ${index + 1}`;
    title = title.replace(/\n/g, ' ').trim();

    // Price — look for Euro patterns
    let price = 0;
    const pricePatterns = [
        /class="[^"]*preis[^"]*"[^>]*>([\s\S]*?)<\/[^>]+>/i,
        /([\d\.\,]+)\s*€/,
    ...
    // Type
    let type = 'Appartamento';
    if (title.toLowerCase().includes('villa')) type = 'Villa';
    else if (title.toLowerCase().includes('fondo') || title.toLowerCase().includes('commerciale')) type = 'Commerciale';
    else if (title.toLowerCase().includes('garage')) type = 'Garage';
    else if (title.toLowerCase().includes('hotel') || title.toLowerCase().includes('resort')) type = 'Hotel';
    ...
    // Description — look for main description block
    let description = '';
    const descPatterns = [
    ...
    // Fallback: grab any large-ish image
    ...
    let localImage = null;
    if (allImgUrls.length > 0) {
        const imgFilename = `scraped_${Date.now()}_${index}.jpg`;
...
async function getAllPropertyLinks() {
    ...
    for (let page = 1; page <= totalPages; page++) {
        const listUrl = `${BASE_URL}/immobilien.xhtml?languag=ITA&p[obj0]=${page}`;
...
async function main() {
    ...
    // Step 1: Get all property links from all 9 pages
